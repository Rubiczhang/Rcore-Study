...
impl Thread {
    ...
    pub fn get_a_copy(&self, context: &mut Context)->MemoryResult<Arc<Thread>>{
        //为何不用self的inner？
        //看上面park prepare两个函数的实现
        let stack = self.process.alloc_page_range(STACK_SIZE, Flags::READABLE | Flags::WRITABLE)?;
        println!("f has been input, forked one thread");
        let mut context = context.clone();
        context.set_sp(stack.end.into());
        //println!("world");
        let thread = Arc::new(Thread {
            id: unsafe {
                THREAD_COUNTER += 1;
